...
public class DoctorPrescriptionFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        context = getActivity();
        view = inflater.inflate(R.layout.fragment_doctor_prescription, container, false);
        init();
        return view;
    }
    ...
    private void getAllPresList() {
        if (Utility.isOnline(context)) {
            final BallTriangleDialog dotDialog = new BallTriangleDialog(context);
            dotDialog.show();
            ServiceCaller serviceCaller = new ServiceCaller(context);
            //here login id....pending
            serviceCaller.callGetAllPrescriptionService(1, new IAsyncWorkCompletedCallback() {
                @Override
