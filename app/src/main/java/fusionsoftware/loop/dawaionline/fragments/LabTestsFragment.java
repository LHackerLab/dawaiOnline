...
public class LabTestsFragment extends Fragment {
    ...
    private void getAllLabTestList() {
        if (Utility.isOnline(context)) {
            final BallTriangleDialog dotDialog = new BallTriangleDialog(context);
            dotDialog.show();
            ServiceCaller serviceCaller = new ServiceCaller(context);
            serviceCaller.callAllLabTestService(new IAsyncWorkCompletedCallback() {
                @Override
                public void onDone(String workName, boolean isComplete) {
                    if (isComplete) {
                        ...
                        if (categoryList != null && categoryList.size() > 0) {
                            LabTestAdapter adapter = new LabTestAdapter(context, categoryList);
