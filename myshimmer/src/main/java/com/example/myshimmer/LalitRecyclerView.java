...
public class LalitRecyclerView extends RecyclerView {
    ...
    private LalitAdapter mShimmerAdapter;
    ...
    public LalitRecyclerView(Context context) {
        super(context);
        init(context, null);
    }


    public LalitRecyclerView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public LalitRecyclerView(Context context, @Nullable AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(context, attrs);
    }

    private void init(Context context, AttributeSet attrs) {
        mShimmerAdapter = new LalitAdapter();
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.LalitRecyclerView, 0, 0);
        ...
        try {
            setDemoLayoutReference(a.getResourceId(R.styleable.LalitRecyclerView_shimmer_demo_layout, R.layout.layout_sample_view));
            ...
            mShimmerAngle = a.getInteger(R.styleable.LalitRecyclerView_shimmer_demo_angle, 0);
            mShimmerColor = a.getColor(R.styleable.LalitRecyclerView_shimmer_demo_shimmer_color, getColor(R.color.colorWhite));
            mShimmerItemBackground = a.getDrawable(R.styleable.LalitRecyclerView_shimmer_demo_view_holder_item_background);
            mShimmerDuration = a.getInteger(R.styleable.LalitRecyclerView_shimmer_demo_duration, 700);
            mShimmerMaskWidth = a.getFloat(R.styleable.LalitRecyclerView_shimmer_demo_mask_width, 0.9f);
            isAnimationReversed = a.getBoolean(R.styleable.LalitRecyclerView_shimmer_demo_reverse_animation, false);
        } finally {
            a.recycle();
        }
